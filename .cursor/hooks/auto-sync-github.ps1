...
try {
    ...
    if ($remote -notmatch 'show0621/sports-quant') {
        Write-Log "Skipped (remote is not show0621/sports-quant): $remote"
        exit 0
    }
    ...
    # 除錯暫存檔不納入版本庫
    git reset HEAD -- "data/_*" 2>$null | Out-Null
    git reset HEAD -- "data/*.log" 2>$null | Out-Null
    ...
    foreach ($file in $staged) {
        ...
        if ($file -like "data/_*" -or $file -like "data/*.log") { $block = $true }
